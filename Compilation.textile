...
Here are instructions for compiling ffmpeg on Mac OS X (tested on Mountain Lion)
http://www.martinlos.com/?p=41
If attempting to compile for media redirection on mac, and enabling ffmpeg using cmake, make sure ffmpeg configure is run with
--arch=x86_64.  

Here is an example of the configuration step before running make for ffmpeg:
bc.
